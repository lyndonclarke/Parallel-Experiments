 ...
 *
 * Compile:  mpicc -g -Wall -o mat_vect_prod mat_vect_prod.c
 * Run:      mpiexec -n <number of processes> mat_vect_prod
 *
 *  1 Node:
 *      500 x 500 = 0.003494
 *      1000 x 1000 = 0.014047
 *      2000 x 2000 = 0.056042
 *  2 Nodes:
 *      500 x 500 = 0.001822
 *      1000 x 1000 = 0.007252
 *      2000 x 2000 = 0.028122
 *  4 Nodes:
 *      500 x 500 = 0.001023
 *      1000 x 1000 = 0.025308
 *      2000 x 2000 = 0.014422
 *
 *
 *
 * Notes:  
...
void Parallel_matrix_vector_prod(float local_A[], int m, 
             ...
             int local_m, int local_n, int my_rank, MPI_Comm comm);
...
//  Print_vector("We read", local_x, local_n, my_rank, p, comm);
    ...
    Parallel_matrix_vector_prod(local_A, m, n, local_x, global_x, 
        local_y, local_m, local_n, my_rank, comm);
    loc_end = MPI_Wtime();
    MPI_Reduce(&loc_start, &all_start, 1, MPI_DOUBLE, MPI_MIN, 0, comm);
    MPI_Reduce(&loc_end, &all_end, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
    if (my_rank == 0){
        all_total = (all_end - all_start);
        printf("The total elapsed time for the calculation was: %lf", all_total);
    }
