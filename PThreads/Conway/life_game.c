 ...
 * 
 * Compile:  gcc -g -Wall -o life life_game.c -lpthreads
 * Run:      ./life <rownThreads> <colThreads> <rows> <cols> <max gens> <'i'|'g'>
...
#  endif
   ...
   wa = malloc(m*n*sizeof(int));
   wb = malloc(m*n*sizeof(int));
   w1 = wa;
   w2 = wb;
   ...
   if (ig == 'i') 
      ...
      Gen_world("What's the prob that a cell is alive?", w1, m, n);

   printf("\n");
   Print_world("Generation 0", w1, m, n);

   for (thread = 0; thread < thread_size; thread++){
      pthread_create(&thread_handles[thread], NULL, Play_life, (void*) thread);
   }
   for (thread = 0; thread < thread_size; thread++){
 ...
 * In args:      thread = calling thread's rank
 * In/out args:  wpp:  pointers to the current world (wpp)
 *
 * Scratch arg:  twpp:  pointer to the next gen world being created (twpp)
 */
void *Play_life(void* thread) {
   ...
   int row_num = m / r;
   int col_num = n / s;
   int first_row = (rank / s) * row_num;
   int first_col = (rank % s) * col_num;

   while (curr_gen < max_gens && dead_world != 1) {
      for (i = first_row; i < (first_row + row_num); i++) {
         for (j = first_col; j < (first_col + col_num); j++) {
...
/*-------------------------------------------------------------------
 * Function:    gen_Barrier
 * Purpose:     Holds all thread in this function until all have entered,
 *				the last one will swap the newly calculated world for the
 *				previous one, print it, and move to the next generation
 * In arg:      rank
...
void *gen_Barrier(void* rank) {
   ...
   if (barrier_thread_count == thread_size) {
